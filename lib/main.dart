...
class _HomePageState extends State<HomePage> {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          ...
          title: const Text('Choose Apps', style: TextStyle(color: Color(0xFFCDD6F4))),
          content: SizedBox(
          ...
          actions: [
            TextButton(
              onPressed: () { saveSettings(); Navigator.pop(ctx); },
              child: const Text('Confirm', style: TextStyle(color: Color(0xFF89B4FA))),
            ),
          ],
        ),
      ),
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('Notifications', style: TextStyle(color: Color(0xFFCDD6F4))),
          ...
          actions: [
            IconButton(icon: const Icon(Icons.refresh, color: Color(0xFF89B4FA)), onPressed: loadData),
            IconButton(icon: const Icon(Icons.apps, color: Color(0xFF89B4FA)), onPressed: showAppSelectionDialog),
          ],
        ),
        body: TabBarView(
          children: [
            notifications.isEmpty
                ? const Center(child: Text('No notifications yet...', style: TextStyle(color: Color(0xFF6C7086), fontSize: 16)))
                : ListView.builder(
                              ...
                              Row(
                                children: [
                                  CircleAvatar(
                                    ...
                                    child: Text((n['app'] as String? ?? '?')[0].toUpperCase(),
                                        style: const TextStyle(color: Color(0xFF1E1E2E), fontWeight: FontWeight.bold, fontSize: 12)),
                                  ),
                                  const SizedBox(width: 8),
                                  Expanded(child: Text('${n['app']}', style: const TextStyle(color: Color(0xFF89B4FA), fontSize: 13, fontWeight: FontWeight.bold))),
                                  Text(formatTime(n['time'] as int? ?? 0), style: const TextStyle(color: Color(0xFF6C7086), fontSize: 12)),
                                ],
                              ),
                              const SizedBox(height: 6),
                              if ((n['title'] as String? ?? '').isNotEmpty)
                                Text(n['title'] as String? ?? '', style: const TextStyle(color: Color(0xFFCDD6F4), fontWeight: FontWeight.bold, fontSize: 14)),
                              const SizedBox(height: 4),
                              Text(n['text'] as String? ?? '', style: const TextStyle(color: Color(0xFFA6ADC8), fontSize: 13)),
                            ],
                          ),
                        ),
                      );
              ...
              children: [
                Container(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Widget Background Opacity', style: TextStyle(color: Color(0xFFCDD6F4), fontSize: 16, fontWeight: FontWeight.bold)),
                      ...
                      Container(
                        ...
                        child: const Center(child: Text('Preview', style: TextStyle(color: Color(0xFFCDD6F4)))),
                      ),
                    ],
                  ),
                ),
                  ...
                  child: Row(
                    ...
                    children: [
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          const Text('Selected Apps', style: TextStyle(color: Color(0xFFCDD6F4), fontSize: 16, fontWeight: FontWeight.bold)),
                          Text(selectedApps.isEmpty ? 'All apps' : '${selectedApps.length} selected', style: const TextStyle(color: Color(0xFF6C7086))),
                        ],
                      ),
                      TextButton(
                        onPressed: showAppSelectionDialog,
                        child: const Text('Edit', style: TextStyle(color: Color(0xFF89B4FA))),
                      ),
                    ],
                  ),
                ),
